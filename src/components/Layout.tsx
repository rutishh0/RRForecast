...
const VIEW_HEADERS: Record<ActiveView, { title: string; subtitle: string }> = {
  ...
  'feature-requests-admin': { title: 'Feature Requests', subtitle: 'Review and manage team feature requests' },
  'user-management': { title: 'User Management', subtitle: 'Add, remove, and manage user accounts' },
};

export default function Layout({ activeView, onViewChange, onUploadClick, onLogout, hasData, currentUser, children }: LayoutProps) {
  const header = VIEW_HEADERS[activeView];

  const [sidebarHovered, setSidebarHovered] = useState<ActiveView | null>(null);
  ...
  const primaryNavItems: NavItem[] = [
    { view: 'my-dashboard', label: 'My Dashboard', icon: Grid },

    { view: 'shop-visits', label: 'Shop Visit Tracker', icon: Wrench },
    { view: 'forecast', label: 'Engine Forecasting', icon: BarChart3 },
    { view: 'data-editor', label: 'Data Editor', icon: PenSquare },
    { view: 'engine-map', label: 'Engine Map', icon: MapPin },
    { view: 'collaborative', label: 'Collaborative', icon: Users },
  ];

  const secondaryNavItems: NavItem[] = [
    { view: 'profile', label: 'Profile', icon: UserIcon },
    { view: 'feature-requests-admin', label: 'Feature Requests', icon: Star, adminOnly: true },
    { view: 'user-management', label: 'User Management', icon: Settings, adminOnly: true },
  ];
      ...
      <aside style={{
        ...
        <nav style={{ flex: 1, padding: "16px 12px", overflowY: "auto", position: "relative", zIndex: 10 }}>
          ...
          {primaryNavItems.map((item, i) => {
            ...
            const isHovered = sidebarHovered === item.view;
            ...
            return (
              <button
                ...
                onMouseEnter={() => setSidebarHovered(item.view)}
