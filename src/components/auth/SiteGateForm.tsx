// V5/src/components/auth/SiteGateForm.tsx
import { useState, type FormEvent } from "react";
import { Loader2, ArrowRight } from "lucide-react";

interface Props {
  visible: boolean;
  onSubmit: (code: string) => void;
  error: string | null;
  loading: boolean;
}

export default function SiteGateForm({ visible, onSubmit, error, loading }: Props) {
  const [code, setCode] = useState("");

  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    if (!code.trim() || loading) return;
    onSubmit(code.trim());
  };

  return (
    <div
      className={`absolute inset-0 transition-all duration-300 ease-out
        ${visible ? "translate-x-0 opacity-100" : "-translate-x-full opacity-0 pointer-events-none"}`}
      aria-hidden={!visible}
    >
      <p className="text-sm text-rr-text-dim mb-6">Enter your access code to continue</p>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="site-code" className="block text-xs uppercase tracking-wider font-semibold text-rr-text-dim mb-2">
            Site Access Code
          </label>
          <input
            id="site-code"
            type="password"
            autoFocus
            value={code}
            onChange={(e) => setCode(e.target.value)}
            disabled={loading}
            className="w-full px-3 py-2 rounded-md bg-rr-navy-50 border border-rr-border
              text-rr-text placeholder:text-rr-text-muted focus:outline-none focus:border-rr-gold/50
              text-sm"
            placeholder="Enter access code"
            autoComplete="off"
          />
          {error && <p className="text-rr-danger text-xs mt-2">{error}</p>}
        </div>

        <button
          type="submit"
          disabled={!code.trim() || loading}
          className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-md
            bg-rr-gold text-rr-bg-secondary font-semibold text-sm
            hover:bg-rr-gold-bright disabled:opacity-50 disabled:cursor-not-allowed
            transition-colors"
        >
          {loading ? (
            <><Loader2 size={14} className="animate-spin" /> Checking…</>
          ) : (
            <>Continue <ArrowRight size={14} /></>
          )}
        </button>
      </form>
    </div>
  );
}
